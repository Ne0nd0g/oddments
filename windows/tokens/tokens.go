...
func PrivilegeCheckN(hToken windows.Token) (hasPriv bool, err error) {
	Advapi32 := windows.NewLazySystemDLL("Advapi32.dll")
	...
	_, _, err = PrivilegeCheck.Call(uintptr(unsafe.Pointer(&hToken)), 0, uintptr(unsafe.Pointer(&hasPriv)))
	if err != syscall.Errno(0){
...
func AdjustTokenPrivilegesG (hToken windows.Token, disable bool, privilege string) (err error){
	...
	if err != nil {
		err = fmt.Errorf("there was an error calling windows.LookupPrivilegeValue: %s", err)
		return
	}
